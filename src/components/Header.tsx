...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 text-white shadow-xl sticky top-0 z-40 border-b border-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex flex-row items-center justify-between gap-3 border-b border-slate-800 overflow-x-auto no-scrollbar">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="relative">
            ...
            {showTournDropdown && (
              <div className="absolute left-0 mt-2 w-72 bg-white text-slate-800 rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="max-h-60 overflow-y-auto py-1 divide-y divide-slate-100/60">
                  {tournaments.map((t) => (
                    <div
                      key={t.id}
        ...
        <div className="flex items-center gap-2 shrink-0 flex-nowrap text-xs">
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white text-slate-800 rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in">
                ...
                <div className="p-1 space-y-1">
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2 text-xs flex items-center gap-2 hover:bg-slate-100 rounded-lg text-slate-700 font-bold transition"
                  >
                    <User className="w-4 h-4 text-slate-500" />
                    <span>Setting Profil & Ubah Password</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2 text-xs flex items-center gap-2 hover:bg-red-50 rounded-lg text-red-600 font-bold transition"
                  >
          ...
          {/* System Settings Button (Only for Master Admin) */}
          {isMaster && (
            <button
              onClick={onOpenSettings}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-slate-900/90">
        <nav className="flex items-center space-x-1 sm:space-x-2 overflow-x-auto py-2.5 no-scrollbar">
          {allTabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => onChangeTab(tab.id)}
              className={`whitespace-nowrap px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition flex items-center gap-2 ${
                activeTab === tab.id
                  ? 'bg-red-600 text-white shadow-lg shadow-red-600/30 font-black tracking-wide transform scale-[1.02]'
